...
export default function AddAppointmentPage() {
  ...
  // Ensure arrays are always defined to avoid calling .filter on undefined
  const {
    employees = [],
    hospitals = [],
    doctors = [],
  ...
  const loadEmployeeData = useCallback(async () => {
    try {
      ...
      if (!foundEmployee) {
        addNotification({ type: 'error', title: 'خطأ', message: 'لم يتم العثور على الموظف' })
      ...
      reset({ appointment_type: 'checkup', appointment_date: '', duration_minutes: 30, hospital_id: '', doctor_id: '', symptoms: '', notes: '' })
    } catch (error) {
      console.error('Load employee data error:', error)
      addNotification({ type: 'error', title: 'خطأ', message: 'حدث خطأ أثناء تحميل بيانات الموظف' })
    } finally {
  ...
  useEffect(() => {
    ...
    loadEmployeeData()
  }, [userData, router, params.id, dataLoading, loadEmployeeData])
  ...
  // Keep available lists in sync when global data loads later
  useEffect(() => {
    const hospitalsList = (hospitals || []).filter(h => h.type === 'hospital')
    setAvailableHospitals(hospitalsList)
        ...
        <Card className="p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 border-b dark:border-gray-800 pb-2">
