...
export default function AddEmployeePage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const { data: employeeData, error: employeeError } = await db.createUser({
      ...
      router.push('/employees?message=تم إضافة الموظف بنجاح')
      
    } catch (error) {
      console.error('Add employee error:', error)
      setError(error.message || 'حدث خطأ أثناء إضافة الموظف')
    } finally {
      setIsLoading(false)
    }
  }
